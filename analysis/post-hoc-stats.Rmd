...
```{r}
HRon.emm <- emmeans(HRon, ~ bout[1,2,3,4,5] |intensity*Gender*Tanner)
HRon.emm
```
It might be nice to extract these estimates and plot them:

```{r}
HRon.emm.df <- HRon.emm %>%
  broom::tidy()
...
RRoff <- lmer(avgRR~ bout*intensity + Gender*Tanner*bout + (bout|ID) , data = avgRRoff, REML=TRUE)
```
